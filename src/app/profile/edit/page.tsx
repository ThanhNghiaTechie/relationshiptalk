...
export default function EditProfilePage() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!/^[a-zA-Z0-9_]{3,30}$/.test(form.username.trim()))
      return setError('Username gồm 3-30 ký tự chữ, số hoặc _.');
    if (form.avatar_url && !/^https?:\/\//.test(form.avatar_url))
      return setError('Avatar phải là một URL hợp lệ.');
      ...
      <div className="w-full max-w-lg rounded-xl border border-border bg-card p-6 shadow-card">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {(['full_name', 'username', 'avatar_url', 'location'] as const).map((field) => (
            <div key={field}>
              <label htmlFor={field} className="mb-1 block text-sm text-muted-foreground">
                {field === 'full_name'
                  ? 'Họ và tên'
                  : field === 'avatar_url'
                    ? 'Avatar URL'
          ...
          <div>
            <label htmlFor="bio" className="mb-1 block text-sm text-muted-foreground">
              Giới thiệu
            </label>
            <textarea
              id="bio"
